...
[`heplot3d()`](https://friendly.github.io/heplots/reference/heplot3d.md),
shown below as a rotatable, zoomable widget. **Challenge Q**: Rotate
this so the \\\mathbf{H}\\ ellipsoid appears as a 1D line (there are
several ways)
...
*3D HE plot for `phyattr`, `independent` and `sophisticated` according
to the classification of photos on `Attr`*
