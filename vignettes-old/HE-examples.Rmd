...
```{r, plastic-univar, results='markup'}
...
```{r, plastic-tests, results='markup'}
...
`hypothesis` argument to `heplot()`, as shown in \figref{fig:plastic2}.
[htb]
...
Finally, a 3D HE plot can be produced with `heplot3d()`, giving \figref{fig:plastic1-HE3D}.
...
pairs of variables, e.g., for `phyattr` and  `happy` (\figref{fig:jury-mod1-HE}).
...
The HE plot for all pairs of variables (\figref{fig:jury-mod1-pairs}) shows that the means for `happy`
...
```{r, jury-can1a, fig.keep='none', results='markup'}
...
```{r, rohwer-separate, results='markup'}
...
```{r, rohwer-mod-test, results='markup'}
...
```
or as a 3D plot, using `heplot3d()` as shown in \figref{fig:rohwer-HE3D}.
...
```{r, rohwer-mod2-test, results='markup'}
...
`SAT` and `PPVT` shown in \figref{fig:rohwer-HE4}. 
...
A `pairs()` plot for the \MLM gives the set of plots shown in \figref{fig:hern-pairs}
...
For example, in the panel for (`leave`, `los`) in  \figref{fig:hern-pairs},
...
the effects of `class` in canonical space, giving \figref{fig:grades-can-class}.
