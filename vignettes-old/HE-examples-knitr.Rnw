...
library(knitr)
opts_chunk$set(
engine='R',fig.height=5,fig.width=5,results='hide',fig.keep='none',echo=TRUE
)
@
...
<<plastic1a, tidy=FALSE, echo=TRUE, eval=FALSE>>=
...
<<plastic1b, tidy=FALSE, echo=TRUE, eval=FALSE>>=
...
@
<<plastic1, fig.keep='none', include=FALSE, echo=FALSE>>=
#      <<plastic1a>>
# Compare evidence and effect scaling 
colors = c("red", "darkblue", "darkgreen", "brown")
heplot(plastic.mod, size="evidence", col=colors, cex=1.25)
...
<<plastic1-HE3D, fig.keep='none', eval=FALSE>>=
...
<<jury-can1a, fig.keep='none', results='markup'>>=
...
<<jury-mod3-eff, include=FALSE, fig.width=8, fig.height=4>>=
library(effects)
...
greater \code{Years} of sentence awarded to Unattractive defendants.
...
<<skulls-bwplot, include=FALSE, fig.height=7, fig.width=7>>=
library(lattice)
library(reshape2)
...
<<skulls-HE-pairs, include=FALSE, tidy=TRUE>>=
...
<<skulls-can2, include=FALSE, fig.width=6, fig.height=6>>=
...
<<rohwer-separate, results='markup'>>=
...
<<rohwer-mod-test, results='markup'>>=
...
<<rohwer-HE2, include=FALSE, tidy=FALSE>>=
...
@
<<rohwer-HE3, include=FALSE, tidy=FALSE>>=
...
The positive orientation of the \code{Regr} ellipses shows that the predicted
...
<<rohwer-HE3, eval=FALSE, tidy=FALSE>>=
...
<<rohwer-HE3D, eval=FALSE, tidy=FALSE>>=
...
<<rohwer-mod2-test, results='markup'>>=
...
<<rohwer-HE4, include=FALSE, tidy=FALSE>>=
colors <- c("red", "blue", rep("black",5), "#969696")
...
body build (\code{build}, a 1-5 scale, with 1=emaciated, \dots, 5=obese),
...
<<hern-can-pstat, include=FALSE, echo=FALSE>>=
plot(Hern.canL, term="pstat")
@
<<hern-can-build, include=FALSE, echo=FALSE>>=
plot(Hern.canL, term="build")
@
<<hern-can-age, include=FALSE, echo=FALSE>>=
plot(Hern.canL, term="age")
@
<<hern-can-cardiac, include=FALSE, echo=FALSE>>=
plot(Hern.canL, term="cardiac")
@
\begin{figure}[htb]
\begin{center}
...
<<grades-HE3D, fig.keep='none', eval=FALSE>>=
...
<<grades-can-class, include=FALSE>>=
...
@
<<grades-can-sex, include=FALSE, echo=FALSE>>=
plot(grades.can, term="sex")
@
<<grades-can-gpa, include=FALSE, echo=FALSE>>=
plot(grades.can, term="gpa")
@
\begin{figure}[htb!]
\begin{center}
