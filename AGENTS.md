...
This file provides guidelines for Codex when working on R packages, particularly focused on statistical visualization and multivariate analysis packages like heplots.
...
I'm not ready to go full-blown `purrr` versions.
...
In `R/` source files, I like to keep track of `TODO` and `FIXME` items, changing the status to `DONE` when they are completed.
