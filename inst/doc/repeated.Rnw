...
%\VignetteDepends{reshape, car, gplots, nlme, lattice}
%\VignetteKeywords{}
...
\Abstract{
	...
	the \pkg{heplots} for \R\ \citep{Fox-etal:07:heplots} and \textsf{SAS}
	...
	the \pkg{heplots} for \R.  Examples describe one-sample profile analysis,
...
\Address{
  ...
  URL: \url{http://www.math.yorku.ca/SCS/friendly.html}
}
...
The \pkg{heplots} package for \R\ \citep{Fox-etal:07:heplots} implements these methods
...
it is important at the outset to understand the aims and scope of this paper.
We are concerned primarily with describing and illustrating a relatively novel,
and hopefully useful, graphical method for visualizing ...
\end{comment}
...
the functions \func{Anova} and \func{linear.hypothesis} in the
\pkg{car}.%
\footnote{
Both the \pkg{car} and the \pkg{heplots} are being actively developed.
Except where noted, all results in this paper were produced using the stable versions on CRAN,
...
In our \R\ implementation, the basic plotting functions in the \pkg{heplots} are \func{heplot}
...
In passing, we note that \emph{all} software (SAS, SPSS, R, etc.)
...
\citep{PinheiroBates:2000,VerbekeMolenberghs:2000}
...
That is, the means across grades 8--11 are equal \emph{iff}
...
\func{linear.hypothesis} can be used to test arbitrary contrasts in the
...
method.%
\footnote{
In contrast, SAS \code{proc glm} and SPSS General Linear Model provide only
...
<<obk5, results=hide, keep.source=TRUE>>=
...
<<obk-HE1, fig=TRUE, include=FALSE, keep.source=TRUE>>=
...
@
This plot is not shown here, but an animated version is included in the supplementary materials.
...
\code{session}.
<<obk6, echo=TRUE, keep.source=TRUE, results=verbatim>>=
...
%}
<<obk-HE3, fig=TRUE, include=FALSE, keep.source=TRUE>>=
...
\begin{eqnarray*}
	\mat{M}_s & = & \mat{M}_{session} \otimes \vec{1}_5 \\
...
\footnote{
These plots were drawn using \func{plotmeans} in the \pkg{gplots}.
...
<<voc-new1,fig=FALSE, echo=TRUE, eval=FALSE, results=hide>>=
(Vocab.mod <- lm(cbind(grade8,grade9,grade10,grade11) ~ 1, data=VocabGrowth))
...
contrasts(session) <- matrix(c(-1,  1, 0,
                                0, -1, 1), ncol=2)
@
<<obk-new1,fig=FALSE, echo=TRUE, results=hide, keep.source=TRUE, eval=FALSE>>=
idata <- data.frame(session)
...
longitudinal models (e.g., \citet[\S 17.4]{VerbekeMolenberghs:2000}).
...
We begin with simple plots of the data: a profile plot grouped by Sex (\figref{fig:ortho-xyplot} (left)),
...
\begin{eqnarray}
  \beta_{0i} & = & \gamma_{00} + \gamma_{01} \mathrm{Sex}_{i} + u_{0i} \\
