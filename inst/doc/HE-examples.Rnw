...
\section[MMRA]{Multivariate Multiple Regression Designs}
...
\begin{figure}[htb]
\begin{center}
	\includegraphics[width=.6\textwidth]{fig/rohwer-HE3D}
...
high-SES groups. Because SES is ``treatment-coded'' in this model, the ellipse for each
covariate represents the hypothesis that the slopes for that covariate are
zero in the high-SES baseline category.
