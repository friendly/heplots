...
Four cases -- 51, 52, 56 and 61 -- are driven to (essentially) zero weight, while all other samples retain weights above 0.7. The weight plot alone doesn't say *why* these four are atypical; for that we turn to the distance plot.

### Distance Plot

```{r pulpfiber-distanceplot, fig.cap="MCD-based distance plot for the robust pulp fiber model: robust distances of the fiber (X) predictors vs. distances of the residuals."}
distancePlot(pulp.rlm, method = "mcd")
```

Using `method = "mcd"` for the predictor distances -- as Rousseeuw et al. [-@Rousseeuw2004] do for this very dataset -- separates the flagged cases into three distinct patterns:
