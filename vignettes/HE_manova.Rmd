...
These relations among three of the variables, `phyattr`, `independent` and
`sophisticated`, can also be viewed directly in 3D with `heplot3d()`,
shown below as a rotatable, zoomable widget.
**Challenge Q**: Rotate this so the $\mathbf{H}$ ellipsoid appears as a 1D line (there are several ways)

```{r, jury-mod1-HE3D-code, eval=FALSE}
heplot3d(jury.mod1, variables = c("phyattr", "independent", "sophisticated"), wire = FALSE)
rgl::rglwidget()
```
```{r, jury-mod1-HE3D, echo=FALSE}
htmltools::tags$iframe(
  src = "fig/jury-mod1-HE3D.html",
  width = "100%", height = "500", style = "border: none;")
```
*3D HE plot for `phyattr`, `independent` and `sophisticated` according to the classification of photos on `Attr`*
